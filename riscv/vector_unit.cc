...
void vectorUnit_t::vectorUnit_t::reset() {
  ...
  csrmap[CSR_VSTART] = vstart =
      std::make_shared<vector_csr_t>(p, CSR_VSTART, /*mask*/ VLEN - 1);
  csrmap[CSR_VXRM] = vxrm =
      std::make_shared<vector_csr_t>(p, CSR_VXRM, /*mask*/ 0x3ul);
  csrmap[CSR_VL] = vl = std::make_shared<vector_csr_t>(p, CSR_VL, /*mask*/ 0);
  csrmap[CSR_VTYPE] = vtype =
      std::make_shared<vector_csr_t>(p, CSR_VTYPE, /*mask*/ 0);
  csrmap[CSR_VLENB] =
      std::make_shared<vector_csr_t>(p, CSR_VLENB, /*mask*/ 0, /*init*/ vlenb);
  assert(VCSR_VXSAT_SHIFT ==
         0); // composite_csr_t assumes vxsat begins at bit 0
  csrmap[CSR_VCSR] = std::make_shared<composite_csr_t>(p, CSR_VCSR, vxrm, vxsat,
                                                       VCSR_VXRM_SHIFT);
...
reg_t vectorUnit_t::vectorUnit_t::set_vl(int rd, int rs1, reg_t reqVL,
                                         reg_t newType) {
  int new_vlmul = 0;
  if (vtype->read() != newType) {
    ...
    vill = !(vflmul >= 0.125 && vflmul <= 8) ||
           vsew > std::min(vflmul, 1.0f) * ELEN || (newType >> 8) != 0;
...
template <class T>
T &vectorUnit_t::elt(reg_t vReg, reg_t n, bool UNUSED is_write) {
  assert(vsew != 0);
...
EG &vectorUnit_t::elt_group(reg_t vReg, reg_t n, bool UNUSED is_write) {
#ifdef WORDS_BIGENDIAN
  fputs(
      "vectorUnit_t::elt_group is not compatible with WORDS_BIGENDIAN setup.\n",
      stderr);
  abort();
#endif
  ...
  const reg_t reg_last =
      vReg + (start_byte + elt_group_size - 1) / bytes_per_reg;
